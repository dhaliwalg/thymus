...
Invariants are defined in `.thymus/invariants.yml` (YAML). A Python parser (`load_invariants()` — duplicated in analyze-edit.sh, scan-project.sh, add-invariant.sh) converts YAML to cached JSON in `/tmp/thymus-cache-{hash}/`. Bash scripts match files against rules using glob-to-regex conversion and grep-based import/pattern detection.
...
- **The YAML parser is duplicated** in three scripts (analyze-edit.sh, scan-project.sh, add-invariant.sh). Any change to parsing logic must be applied to all three. Search for `def parse(src, dst):` to find all instances. A fourth, simplified parser in refresh-baseline.sh (`_update_claude_md`) extracts only `id`, `description`, and `severity` — update its regexes if the YAML format changes.
...
- **The `glob_to_regex()` function is also duplicated** across analyze-edit.sh and scan-project.sh. Keep them in sync.
...
1. Add the case branch in `analyze-edit.sh` (single-file check) and `scan-project.sh` (batch check).
...
2. Update `extract_ts_imports()` (or add a language-specific equivalent) in both `analyze-edit.sh` and `scan-project.sh`.
3. Add a test fixture under `tests/fixtures/` with language-specific invariants.
...
- JSON output uses `jq -n` for construction. Never echo raw JSON strings.
